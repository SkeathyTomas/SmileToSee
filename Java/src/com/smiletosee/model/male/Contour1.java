...
public class Contour1 {
    public static void main(String [] args) throws Exception {
        String path = "F:/study/SRTP/模型训练/classified/男/contour1/contour1.csv";
        ...
        //结果变量
        for (int i=0;i<8;i++){
            if(csvList.get(i+10)[38].equals("a"))
                lables[i] = 0;
            if (csvList.get(i+10)[38].equals("b"))
                lables[i] = 1;
        }
        //特征变量
        for (int i=0;i<8;i++){
            for (int j=0;j<38;j++){
                data[i][j] = Double.valueOf(csvList.get(i+10)[j]);
            }
        }
